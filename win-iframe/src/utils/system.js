...
(function () {
   ...
   * Provides
   * - SystemJS.import
   * - SystemJS.register support for
   *     live bindings, function hoisting through circular references,
   *     reexports, dynamic import, import.meta.url, top-level await
   * - SystemJS.getRegister to get the registration
   * - Symbol.toStringTag support in Module objects
   * - Hookable SystemJS.createContext to customize import.meta
   * - SystemJS.onload(err, id, deps) handler for tracing / hot-reloading
   * 
   * Core comes with no SystemJS.prototype.resolve or
   * SystemJS.prototype.instantiate implementations
   */
  ...
  /*
   * getRegister provides the last anonymous SystemJS.register call
   */
  ...
  envGlobal.SystemJS = new SystemJS();
  ...
  /*
   * Supports loading SystemJS.register via script tag injection
   */
  ...
  function loadScriptModules() {
    Array.prototype.forEach.call(
      document.querySelectorAll('script[type=systemjs-module]'), function (script) {
        if (script.src) {
          SystemJS.import(script.src.slice(0, 7) === 'import:' ? script.src.slice(7) : resolveUrl(script.src, baseUrl));
        }
      });
  }

  /*
   * Supports loading SystemJS.register in workers
   */
   ...
   * (Included by default in SystemJS.js build)
   */
  (function (global) {
    const systemJSPrototype = global.SystemJS.constructor.prototype;
